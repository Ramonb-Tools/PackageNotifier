...
async function initializeAuth() {
  if (!isConfigured()) {
    showStatus(
      ...
      "Add the approved Microsoft application configuration in config.js before Outlook sign-in can work."
    );
    return;
  }
...
async function signIn() {
  if (!isConfigured()) {
    showStatus(
      "CONFIGURATION REQUIRED",
      "The Outlook integration has not been configured yet. Please use the approved organization configuration."
    );
    return;
  }
